...
*Setup instructions for a gitian build of MN Browsing Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MN Browsing Coin
...
- [Building MN Browsing Coin](#building-mnbrowsingcoin)
...
Clone the git repositories for mnbrowsingcoin and gitian and then checkout the mnbrowsingcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mnbrowsingcoin-crypto/mnbrowsingcoin.git
cd mnbrowsingcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mnbrowsingcoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building MN Browsing Coin
----------------

To build MN Browsing Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mnbrowsingcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mnbrowsingcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mnbrowsingcoin-crypto/mnbrowsingcoin
...
For example:
```bash
URL=https://github.com/crowning-/mnbrowsingcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mnbrowsingcoin=${COMMIT} --url mnbrowsingcoin=${URL} ../mnbrowsingcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mnbrowsingcoin=${COMMIT} --url mnbrowsingcoin=${URL} ../mnbrowsingcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mnbrowsingcoin=${COMMIT} --url mnbrowsingcoin=${URL} ../mnbrowsingcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mnbrowsingcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mnbrowsingcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mnbrowsingcoin-build.assert
```
...
[mnbrowsingcoin/gitian.sigs](https://github.com/mnbrowsingcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
