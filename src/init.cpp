...
#include "config/mnbrowsingcoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("mnbrowsingcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mnbrowsingcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mnbrowsingcoind.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, mnbrowsingcoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MNBC specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizemnbrowsingcoinamount=<n>", strprintf(_("Keep N PIV anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
	   FormatParagraph(strprintf(_("Copyright (C) 2017-%i The MN Browsing Coin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("mnbrowsingcoin-loadblk");
...
/** Sanity checks
 *  Ensure that MN Browsing Coin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MN Browsing Coin Core is shutting down."));
...
#endif
    // Make sure only a single MN Browsing Coin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MN Browsing Coin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MN Browsing Coin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MN Browsing Coin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MN Browsing Coin Core to complete") << "\n";
    ...
    nAnonymizeMNBrowsingCoinAmount = GetArg("-anonymizemnbrowsingcoinamount", 0);
    if (nAnonymizeMNBrowsingCoinAmount > 999999) nAnonymizeMNBrowsingCoinAmount = 999999;
    if (nAnonymizeMNBrowsingCoinAmount < 2) nAnonymizeMNBrowsingCoinAmount = 2;
    ...
    LogPrintf("Anonymize MN Browsing Coin Amount %d\n", nAnonymizeMNBrowsingCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1MNBC+1000 == (.1MNBC+100)*10
       10MNBC+10000 == (1MNBC+1000)*10
    */
